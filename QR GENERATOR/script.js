...
function fetchRequest(file, formData) {
    ...
    fetch("http://api.qrserver.com/v1/read-qr-code/", {
        ...
        infoText.innerText = result ? `Choose your QR code image and decode it!!` : "Choose your QR code image and decode it!!"
        if(!result) return;
        document.querySelector("textarea").innerText = result;
        form.querySelector("img").src = URL.createObjectURL(file);
        readerBox.classList.add("active");
    }).catch(() => {
        infoText.innerText = "Choose your QR code image and decode it!!";
...
submitBtn.addEventListener("click", async () => {
  ...
  const src = showQr.firstChild.toDataURL("image/png");
  downloadBtn.href = src;
  let userValue = 'Qr code';
  try {
    userValue = new URL(userValue).hostname;
  } catch (_) {
    userValue = inputFormatter(userValue);
    downloadBtn.download = `${userValue}Image`;
